...
public class CommonStaffController {
    ...
    public String sendOtp(@RequestParam String email, @RequestParam String role, Model model) {
        ...
        try {
            ...
            if (staffAuth == null) {
                model.addAttribute("errorPopup", "Email not registered! Please contact Admin\n Redirecting to home...");
                model.addAttribute("redirectIndex", true);
                return "staffs/commonLogin";
            }
            ...
            if (!roleMatches) {
                model.addAttribute("errorPopup",
                        "This email does not belong to " + capitalize(role) + ". Redirecting...");
                model.addAttribute("redirectIndex", true);
                return "staffs/commonLogin";
            }
    ...
    public String verifyOtp(@RequestParam String email, @RequestParam String otp, @RequestParam String role,
        ...
        if (otp.equals(generatedOtp)) {
            ...
            model.addAttribute("email", email);
            model.addAttribute("role", role);
            model.addAttribute("errorPopup", "Invalid OTP! Redirecting...");
            model.addAttribute("redirectIndex", true);
        }
        return "staffs/commonLogin";
    }
    ...
    public String setPassword(@RequestParam String email,
        ...
        if (staff != null) {
            ...
            // Add passwordSet attribute for popup
            model.addAttribute("passwordSet", "Password set successfully! Redirecting...");

            // Keep role and email for Thymeleaf forms
            model.addAttribute("role", capitalize(role));
            model.addAttribute("email", email);
        ...
        // Return same login page so popup shows
        return "staffs/commonLogin";
    }
    ...
    public String handleLogin(
        ...
        if (staffAuth != null) {
            ...
            if (!roleMatches || !passwordMatches) {
                model.addAttribute("errorPopup", "Invalid email or password! Redirecting...");
                model.addAttribute("redirectIndex", true);
                return "staffs/commonLogin";
            }
